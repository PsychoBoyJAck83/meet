...
defineFeature(feature, (test) => {
    ...
    then(
      ...
      () => {
        ...
        eventElements.forEach((eventElement) => {
          const details =
            within(eventElement).queryByTestId("event-description");
  ...
  test("User can expand event details", ({ given, when, then }) => {
    ...
    given(
      "the user opened the application and the list of upcoming events was displayed",
      async () => {
        render(<App />);
        EventListDOM = screen.getByTestId("event-list");
    ...
    when("the user clicks on show details button", async () => {
      ...
      const showDetailsButton =
        within(eventElement).queryByTestId("details-btn");
    ...
    then(
      ...
      () => {
        const details =
          within(expandedEventElement).queryByTestId("event-description");
  ...
  test("User can hide event details", ({ given, when, then }) => {
    ...
    let EventListDOM;
    given(
      "the user opened the application and clicked the show details button of the first event",
      () => {
        render(<App />);
        EventListDOM = screen.getByTestId("event-list");
    ...
    when(
      "the user clicks the details button again, while details being shown",
      () => {
        const detailsButton =
          within(expandedEventElement).queryByTestId("details-btn");
        userEvent.click(detailsButton);
      }
    );
